...
DEFAULT_DATABASE_URL = "postgresql://postgres@localhost:5432/mbm"
...
def read_session_secret() -> str:
    session_secret = os.environ.get("MBM_SESSION_SECRET", None)

    if not session_secret:
        if FLASK_ENV in DEVELOPMENT_ENVS:
            # Allow omitting in development, use a fixed secret instead.
            session_secret = f"mbm-{FLASK_ENV}-session-secret-v1"
        else:
            raise Exception(
                "MBM_SESSION_SECRET env var for managing sessions is missing"
            )
...
def read_http_origin() -> str:
    http_origin = os.environ.get("MBM_HTTP_ORIGIN", None)
    ...
    if not http_origin:
        ...
        # request, we need to create the http origin based on the app name.
            ...
            raise Exception(
                "MBM_HTTP_ORIGIN env var, e.g. https://mbm.example.com, is missing"
            )
